...
public class SystemUtilities {
    ...
    /**
     * Set the path provider for accessing installation paths.
    ...
    public static boolean isUnix() {
        if(OS_NAME != null) {
            ...
            return OS_TYPE == Os.SOLARIS || OS_TYPE == Os.LINUX ||
    ...
    public static boolean prefersMaskTrayIcon() {
        if (Constants.MASK_TRAY_SUPPORTED) {
            if (SystemUtilities.isWindows() && SystemUtilities.getOsVersion().getMajorVersion() >= 10) {
                return true;
            }
        }
        return false;
    ...
    public static boolean setSystemLookAndFeel(boolean headless) {
        ...
        try {
            ...
            // Use FlatLaf macOS Light theme for a modern look and feel
            try {
                // Import and setup FlatLaf
                Class<?> flatMacLightLafClass = Class.forName("com.formdev.flatlaf.themes.FlatMacLightLaf");
                Class<?> flatMacDarkLafClass = Class.forName("com.formdev.flatlaf.themes.FlatMacDarkLaf");
                
                // Choose light or dark theme based on system settings
                Class<?> lafClass = isDarkDesktop() ? flatMacDarkLafClass : flatMacLightLafClass;
                Method setupMethod = lafClass.getMethod("setup");
                setupMethod.invoke(null);
                
                // Apply macOS style customizations
                UIManager.put("Button.arc", 6);
                UIManager.put("Component.arc", 6);
                UIManager.put("TextComponent.arc", 6);
                UIManager.put("Component.focusWidth", 2);
                UIManager.put("Component.focusColor", new Color(0, 103, 224)); // macOS accent blue
                ...
                log.info("FlatLaf look and feel applied successfully");
            } catch (Exception e) {
                log.warn("Failed to apply FlatLaf look and feel, falling back to default", e);
            ...
            log.warn("Error getting the default look and feel", t);
        }
        return false;
    }
     ...
     * - First byte is a salted version of the timestamp
    ...
    private static long calculateChallenge() {
        ...
        if(SystemUtilities.isJar()) {
            ...
            if (pathProvider != null) {
                jarPath = Paths.get(pathProvider.getInstallationDestination(), "app.jar");
            } else {
                log.warn("No path provider configured, using fallback path");
                jarPath = Paths.get(".", "app.jar");
            }
        }
     ...
     * - If unsalted value of first byte matches the timestamp, return true
