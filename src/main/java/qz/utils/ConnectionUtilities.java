...
public final class ConnectionUtilities {
    ...
    public static InputStream getInputStream(String urlString, boolean protocolRestricted) throws IOException {
        try {
            ...
            // If in development mode and this is an HTTPS connection, bypass SSL verification
            if (urlConn instanceof HttpsURLConnection) {
                setupTrustAllSSL((HttpsURLConnection)urlConn);
                log.warn("DEV MODE: SSL certificate verification bypassed for {}", urlString);
            }
    ...
    /**
     * Sets up an HTTPS connection to trust all certificates
     * 
     * @param connection the HTTPS connection to configure
     */
    private static void setupTrustAllSSL(HttpsURLConnection connection) {
        try {
    ...
    private static Map<String, String> getRequestProperties() {
        if (requestProps == null) {
            ...
            requestProps.put("User-Agent", String.format("Mozilla/5.0 (%s; %s) %s/%s",
                                                         getUserAgentOS(),
                                                         getUserAgentArch(),
                                                         getFrameworkName(),
            ...
            requestProps.put("Sec-CH-UA", String.format("\"%s\"; v=\"%s\"",
                                                        getFrameworkName(),
                                                        getFrameworkVersion()));
