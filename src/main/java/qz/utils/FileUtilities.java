...
import qz.common.CertificateProvider;
...
public class FileUtilities {
    ...
    private static SystemFoldersProvider systemFoldersProvider;
    private static CertificateProvider certificateProvider;
    private static ProvisionProvider provisionProvider;
    
    // Setter methods for provider injection
    public static void setSystemFoldersProvider(SystemFoldersProvider provider) {
        systemFoldersProvider = provider;
    }
    ...
    public static void setProvisionProvider(ProvisionProvider provider) {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attributes) {
                if (file.getFileName().toString().endsWith(".log")) {
                    try {
                        ...
                        log.warn("Unable to add file {} to zip file {}", file, outputFile, e);
                    }
    ...
    private static Path getUserDesktop() {
        ...
        if (SystemUtilities.isWindows()) {
            try {
                if (systemFoldersProvider != null) {
                    return Paths.get(systemFoldersProvider.getDesktopPath());
                } else {
                    log.warn("SystemFoldersProvider not set, falling back to user.home");
                }
    ...
    private static Path getUserDirectory() {
        if (SystemUtilities.isWindows()) {
            try {
                if (systemFoldersProvider != null) {
                    return Paths.get(systemFoldersProvider.getRoamingAppDataPath(), Constants.DATA_DIR);
                } else {
    ...
    private static Path getSharedDirectory() {
        if (SystemUtilities.isWindows()) {
            try {
                if (systemFoldersProvider != null) {
                    return Paths.get(systemFoldersProvider.getProgramDataPath(), Constants.DATA_DIR);
                } else {
        ...
        if (SystemUtilities.isWindows()) {
            ...
            try {
                FileAttribute<Set<PosixFilePermission>> attributes = PosixFilePermissions
                        .asFileAttribute(Files.getPosixFilePermissions(filePath.getParent()));
    ...
    private static final String[] badExtensions = new String[] { "exe", "pif", "paf", "application", "msi", "com",
            "cmd", "bat", "lnk", // Windows Executable program or script
            ...
            "ps1", "ps1xml", "ps2", "ps2xml", "ps1", "ps1xml", "ps2", "ps2xml", "psc1", "psc2", // Windows PowerShell
                                                                                                // script
            ...
            "ppt", "pps", "pptx", "pptm", "potx", "potm", "ppam", "ppsx", "ppsm", "sldx", "sldm", // Microsoft
                                                                                                  // PowerPoint
    ...
    private static final CharSequenceTranslator translator = new LookupTranslator(
            new String[][] { { "" + ESCAPE_CHAR, "" + ESCAPE_CHAR + ESCAPE_CHAR }, { "\\", ESCAPE_CHAR + "b" },
                    { "/", ESCAPE_CHAR + "f" }, { ":", ESCAPE_CHAR + "c" }, { "*", ESCAPE_CHAR + "a" },
                    { "?", ESCAPE_CHAR + "m" }, { "\"", ESCAPE_CHAR + "q" }, { "<", ESCAPE_CHAR + "g" },
                    { ">", ESCAPE_CHAR + "l" }, { "|", ESCAPE_CHAR + "p" }, { "" + (char) 0x7f, ESCAPE_CHAR + "d" } });
    ...
    // File IO settings removed - methods deleted

    /**
     * Performs security checks before allowing File IO operations: 1. Is the
     * request verified (was the signature OK?)? 2. Is the certificate valid? 3. Is
     * the location whitelisted? 4. Is the file extension permitted
     */
    // File operations removed - method deleted

    public static Path getAbsolutePath(JSONObject params, RequestState request, boolean allowRootDir)
            throws JSONException, IOException {
        return getAbsolutePath(params, request, allowRootDir, false);
    }

    public static Path getAbsolutePath(JSONObject params, RequestState request, boolean allowRootDir,
            boolean createMissing) throws JSONException, IOException {
        // File operations removed - method returns null
        return null;
    }

    /**
     * Returns a normalised and absolute path. If the input path was relative, the
     * root may reside in one of four locations, based on the sandbox and shared
     * flags. If the input path was absolute, the path will be normalised and
     * returned without any further changes.
     *
     * @param filePath File or Directory path (no longer used)
     * @param commonName Common name of the associated certificate (no longer used)
     * @return null - file operations disabled
     */
    public static Path createAbsolutePath(Object filePath, String commonName) {
        // File operations removed - method returns null
        return null;
    }

    /**
     * Checks a path's extension against a list of forbidden extensions. If a match
     * is found, false is returned.
     */
    public static boolean isGoodExtension(Path path) {
        String fileName = path.getFileName().toString();

        // "foo.exe." is valid on windows, but is immediately changed to "foo.exe" by
        // the os
        // this is undocumented behavior, therefore, rather than trying to support it,
        // we fail it.
        if (SystemUtilities.isWindows() && fileName.endsWith(".")) {
            return false;
        }
     ...
     * Currently hard-coded to the QZ data directory or anything provided by
     */
    public static boolean isWhiteListed(Path path, boolean allowRootDir, boolean sandbox, RequestState request) {
        ...
        if (whiteList == null) {
            ...
            // whiteList.addAll(parseDelimitedPaths(getFileAllowProperty(AppContext.getTrayProperties()).toString()));
        }
        ...
        for (Map.Entry<Path, String> allowed : whiteList) {
            if (cleanPath.startsWith(allowed.getKey())) {
                if ("".equals(allowed.getValue())
                        || allowed.getValue().contains(FIELD_SEPARATOR + commonName + FIELD_SEPARATOR)
                                && (allowRootDir || !cleanPath.equals(allowed.getKey()))) {
                    return true;
                } else if (allowed.getValue().contains(FIELD_SEPARATOR + "sandbox" + FIELD_SEPARATOR)) {
                    Path p;
                    if (sandbox) {
                        p = Paths.get(allowed.getKey().toString(), "sandbox", commonName);
                    } else {
                        p = Paths.get(allowed.getKey().toString(), "data");
                    }
    ...
    private static boolean saveFileAllowProperty(PropertyHelper props, ArrayList<Map.Entry<Path, String>> paths) {
        ...
        for (Map.Entry<Path, String> path : paths) {
            fileAllow.append(escapePathProperty(path.getKey().toString())).append(path.getValue())
    ...
    /**
     * Escapes <code>ESCAPE_CHARACTER</code>, <code>FILE_SEPARATOR</code> and
     * <code>FIELD_SEPARATOR</code> so it a multi-value file path can be safely
     * parsed later
     */
    private static String escapePathProperty(String path) {
        return path == null ? ""
                : path.replace("" + ESCAPE_CHAR, ("" + ESCAPE_CHAR) + ESCAPE_CHAR)
    ...
    private static StringBuilder getFileAllowProperty(Properties props) {
        ...
        if (props != null) {
            ...
            if (propString.length() == 0) {
                ...
                if (propString.length() > 0) {
                    log.warn(
                            "Property \"file.whitelist\" is deprecated and will be removed in a future version.  Please use \"file.allow\" instead.");
                }
            }
        }
        return propString;
     ...
     * e.g. C:\file1.txt;C:\file2.txt C:\file1.txt|ABC Inc.;C:\file2.txt|XYZ Inc.
     */
    public static ArrayList<Map.Entry<Path, String>> parseDelimitedPaths(String delimited, boolean escapeCommonNames) {
        ...
        if (delimited != null) {
            ...
            for (int i = 0; i < propString.length(); i++) {
                ...
                // if the char before this was an escape char, we are no longer escaped and we
                // skip delimiter detection
                if (escaped) {
                    escaped = false;
                } else {
                ...
                // if a semicolon was found or we are on the last char of the string, dump the
                // tokens into a pair and add it to whiteList
                if (resetPending || lastChar) {
                    ...
                    foundPaths.add(new AbstractMap.SimpleEntry<>(Paths.get(tokens.get(0)).normalize().toAbsolutePath(),
                            commonNames));
                    tokens.clear();
                }
            }
    ...
    /**
     * Returns whether or not the supplied path is restricted.
    ...
    /**
     * Escapes invalid chars from filenames. This does not cause collisions. Escape
     * char is <code>ESCAPE_CHAR</code> Characters escaped, ^ \ / : * ? " < > |</>
     * Warning: Restricted filenames such as lpt1, com1, aux... are not escaped by
     * this function
     *
    ...
    public static void setupListener(Object fileListener) throws IOException {
        // File operations removed - method does nothing (file listener parameter no longer used)
    }

    /**
     * Reads an XML file from URL, searches for the tag specified by {@code dataTag}
     * tag name and returns the {@code String} value of that tag.
     *
     ...
     */
    public static String readXMLFile(String url, String dataTag)
            throws DOMException, IOException, NullCommandException, ParserConfigurationException, SAXException {
    ...
    public static ArgParser.ExitStatus addToCertList(String list, File certFile) throws Exception {
        ...
        if (FileUtilities.printLineToFile(list, cert.data(), !SystemUtilities.isAdmin())) {
            log.info("Successfully added {} to {}", cert.getOrganization(), list);
            return ArgParser.ExitStatus.SUCCESS;
        }
        log.error("Failed to add {} to {} list", cert.getOrganization(), list);
    ...
    /**
     *
     * @return First line of ".autostart" file in user or shared space or "0" if
     *         blank. If neither are found, returns "1".
     * @throws IOException
     */
    ...
    private static synchronized boolean writeAutoStartFile(String mode) throws IOException {
        ...
        Files.write(autostartFile, mode.getBytes(), StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.CREATE,
    ...
    /**
     * Configures the given embedded resource file using Constants
     * combined with the provided HashMap and writes to the specified location
     *
     * Will look for resource relative to relativeClass package location.
     */
    public static synchronized void configureAssetFile(String relativeAsset, File dest,
            HashMap<String, String> additionalMappings, Class relativeClass) throws IOException {
        ...
        BufferedReader reader = new BufferedReader(
                new InputStreamReader(relativeClass.getResourceAsStream(relativeAsset)));
    ...
    public static void configureAssetFile(String relativeAsset, Path dest, HashMap<String, String> additionalMappings,
            Class relativeClass) throws IOException {
    ...
    public static void setPermissionsParentally(Path toTraverse, boolean worldWrite) {
        ...
        // Assume we shouldn't go higher than 2nd-level (e.g. "/etc", "C:\Program
        // Files\", etc)
    ...
    public static void setPermissionsRecursively(Path toRecurse, boolean worldWrite) {
        try (Stream<Path> paths = Files.walk(toRecurse)) {
            paths.forEach((path) -> {
                if (SystemUtilities.isWindows() && worldWrite) {
                    // By default, NSIS sets owner to "Administrator", preventing non-admins from
                    // writing
