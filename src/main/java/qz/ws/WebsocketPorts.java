...
public class WebsocketPorts {
    ...
    /**
     * Hook method to parse property pairs from a step. 
     * This method can be overridden by subclasses to provide different implementations.
     */
    protected static HashMap<String, String> parsePropertyPairs(Step step) {
        // Default implementation - empty map since base class doesn't have PropertyInvoker
        return new HashMap<>();
    }

    /**
     * Loops through steps searching for a property which sets a custom websocket ports
     * Uses a property parser function to handle Step parsing
     */
    public static WebsocketPorts parseFromSteps(List<Step> steps, java.util.function.Function<Step, HashMap<String, String>> propertyParser) {
        List<Integer> secure = new ArrayList<>();
        List<Integer> insecure = new ArrayList<>();
        for(Step step : steps) {
            if(step.getType() == Type.PROPERTY) {
                HashMap<String, String> pairs = propertyParser.apply(step);
                String foundPorts;
    ...
    /**
     * Loops through steps searching for a property which sets a custom websocket ports
     * Uses default property parsing (returns empty map)
     */
    public static WebsocketPorts parseFromSteps(List<Step> steps) {
        return parseFromSteps(steps, WebsocketPorts::parsePropertyPairs);
    }
