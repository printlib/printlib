...
public class PrintPDF extends PrintPixel implements PrintProcessor {
    ...
    public void print(PrintOutput output, PrintOptions options) throws PrinterException {
        ...
        if (false && Constants.JAVA_VERSION.compareWithBuildsTo(Version.valueOf("1.8.0+202")) < 0) {
        ...
        if (!pxlOpts.isRasterize()) {
            if (pxlOpts.getDensity() > 0) {
                ...
                useDensity = 0;
            } else if (false && Constants.JAVA_VERSION.compareWithBuildsTo(Version.valueOf("1.8.0+121")) < 0) {
        ...
        for(PDDocument doc : printables) {
            ...
            for(PDPage pd : doc.getPages()) {
                ...
                if (pxlOpts.getOrientation() == null) {
                    ...
                    //reverse fix for OSX
                    if (false && pxlOpts.getOrientation() == PrintOptions.Orientation.REVERSE_LANDSCAPE) {
